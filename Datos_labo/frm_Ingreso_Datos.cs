...
namespace Registro_Heladera2
{
    public partial class Form1 : Form
    {
           ...
           //while (string.IsNullOrWhiteSpace(txt_Usuario.Text) && flag )
            ...
            if (txt_Usuario.Text.Length > 0)
            {
...

                {
                    ...
                    //****************************************************
                    string archivo = path + "\\" + lbl_Main.Text + ".txt";
                    string archivo_alarma = path + "\\" + lbl_Main.Text + "_alarma.txt" ;
                    string resumen_alarmas = path + "\\" + lbl_Main.Text + "_resumen_alarmas.txt";
                    ...
                    if (flag_temp && flag_humedad && flag_hora)
                    {
                        ...
                        if ((temperatura < 2) || (temperatura > 8))
                        {
                            ...
                            if (resultado == DialogResult.Yes)
                            {
                                ...
                                string linea = str + ";" + txt_Hora.Text + ";" + dtP_Fecha.Text + ";" + System.Convert.ToString(DateTime.Now) + ";" + Environment.NewLine;
                                File.AppendAllText(archivo , linea);
                                funciones.add_alarm(archivo_alarma, linea);
                                // Envía mail !!! Si, lo envía copia la alarma al Resumen_Alarmas, y borra la alarma 
                                if (funciones.Send_email_from_gmail(<hardcoded GMAIL account to send notification >, Path.Combine( path , "mail.txt" )  , lbl_Main.Text , archivo_alarma))
                                    {

                                        string[] alarmas_anteriores = File.ReadAllLines( archivo_alarma );
                                        
                                        File.AppendAllLines( resumen_alarmas , alarmas_anteriores );
                                        
                                        File.Delete(archivo_alarma);
                                    }
        ...
        private void lbl_Main_Click(object sender, EventArgs e)
        {
...
/*
    /// <summary>
    ///////---------------------Comienzode clase agregada
    /// </summary> extraído de http://stackoverflow.com/questions/15569641/reset-all-the-items-in-a-form
